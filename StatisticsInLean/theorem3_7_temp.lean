...
abbrev root_of_deriv (f : ℝ → EReal):= {(θ: ℝ) | deriv (fun x => (f x).toReal) θ = 0}
...
theorem exists_consistent_estimator_of_logLikelihood
  ...
  ∃ (θ_hat : ℕ → Ω → ℝ),
    Tendsto (fun i => (f θ₀).1 { ω | edist (θ_hat i ω) θ₀ < a }) atTop (𝓝 1) := by
  ...
  have hA : Tendsto (fun k => P (A k)) atTop (𝓝 1) := by
    ...
    have hmsU : ∀ k, MeasurableSet (AU k) := by
      intro k
      simpa [AU, gt_iff_lt] using (measurableSet_lt (Measurable_log_Likelihood f μ X θU k)
        (Measurable_log_Likelihood f μ X θ₀ k))

    have hmsL : ∀ k, MeasurableSet (AL k) := by
      intro k
      simpa [AL, gt_iff_lt] using (measurableSet_lt (Measurable_log_Likelihood f μ X θL k)
        (Measurable_log_Likelihood f μ X θ₀ k))
  ...
  have hcontR :
      ∀ (k : ℕ),
      ∀ (ω : Ω), ContinuousOn (fun θ => (log_Likelihood f X θ k μ ω).toReal) I := by
    intro k ω
    ...
    if h : (ω ∈ AU k) ∧ (ω ∈ AL k) then
      Classical.choose
        (exists_deriv_eq_zero_of_strict_endpoints
          ...
          ha ha_fin
          (by
            have : ContinuousOn (fun θ => (log_Likelihood f X θ k μ ω).toReal) I := by
              exact hcontR k ω
            simpa [I] using this)
          (by
            have hfin0 := hfinite k ω θ₀ (by
              simp [I] )
            have hfinU := hfinite k ω (θ₀ + a.toReal) (by
              simp [I, add_comm, add_left_comm] )
            have : (log_Likelihood f X (θ₀ + a.toReal) k μ ω).toReal
                < (log_Likelihood f X θ₀ k μ ω).toReal := by
              exact EReal.toReal_lt_toReal (fun a_1 ↦ hfl k (θ₀ + a.toReal) ω (id (Eq.symm a_1)))
                (hfs k (θ₀ + a.toReal) ω) (hfs k θ₀ ω) (fun a ↦ hfl k θ₀ ω (id (Eq.symm a)))
                (by simpa [AU, θU] using h.1)
            simpa [θU] using this)
          (by
            have hfin0 := hfinite k ω θ₀ (by simp [I])
            have hfinL := hfinite k ω (θ₀ - a.toReal)
              (by simp_rw [I, Set.mem_Icc, le_refl, tsub_le_iff_right, true_and, add_assoc,
                le_add_iff_nonneg_right, nonneg_add_self_iff, ENNReal.toReal_nonneg])
            have : (log_Likelihood f X (θ₀ - a.toReal) k μ ω).toReal
                < (log_Likelihood f X θ₀ k μ ω).toReal := by
              exact EReal.toReal_lt_toReal (fun a_1 ↦ hfl k (θ₀ - a.toReal) ω (id (Eq.symm a_1)))
                (hfs k (θ₀ - a.toReal) ω) (hfs k θ₀ ω) (fun a ↦ hfl k θ₀ ω (id (Eq.symm a)))
      ...
      θ₀, ?_⟩
  let T : ℕ → Set Ω := fun k =>
    {ω : Ω | edist (if h : (ω ∈ AU k) ∧ (ω ∈ AL k) then
      Classical.choose
        (exists_deriv_eq_zero_of_strict_endpoints
          (g := fun θ => (log_Likelihood f X θ k μ ω).toReal)
          (θ₀ := θ₀) (a := a)
          ...
          (by
            have : ContinuousOn (fun θ => (log_Likelihood f X θ k μ ω).toReal) I := by
              refine (EReal.continuousOn_toReal).comp (hcont k ω) ?_
              intro θ hθ
              simp only [Set.mem_compl_iff, Set.mem_insert_iff, Set.mem_singleton_iff, not_or]
              refine ⟨fun a ↦ hfl k θ ω (id (Eq.symm a)), hfs k θ ω⟩
            simpa [I] using this)
          (by
            simp only [gt_iff_lt]
  ...
  have hsubset : ∀ k, A k ⊆ T k := by
    intro k ω hω
    have h : (ω ∈ AU k) ∧ (ω ∈ AL k) := hω
    simp only [A, AU, AL, T,
      gt_iff_lt, ne_eq, Set.mem_inter_iff, Set.mem_setOf_eq] at *
    have hs :=
      (Classical.choose_spec
        (exists_deriv_eq_zero_of_strict_endpoints
          ...
          (by
            simp only [gt_iff_lt]
            exact EReal.toReal_lt_toReal (fun a_1 ↦ hfl k (θ₀ + a.toReal)
              ω (id (Eq.symm a_1)))
              (hfs k (θ₀ + a.toReal) ω)
              (hfs k θ₀ ω)
              (fun a ↦ hfl k θ₀ ω (id (Eq.symm a)))
          ...
          (by
            simp only [gt_iff_lt]
            exact EReal.toReal_lt_toReal (fun a_1 ↦ hfl k (θ₀ - a.toReal)
              ω (id (Eq.symm a_1))) (hfs k (θ₀ - a.toReal) ω) (hfs k θ₀ ω)
                  (fun a ↦ hfl k θ₀ ω (id (Eq.symm a))) (by simpa [AL, θL] using h.2))))
    simp only [h]
    exact hs.1
  ...
  exact tendsto_of_tendsto_of_tendsto_of_le_of_le hA (univ_tendsto_one P) (fun k => hmono k)
    (fun k => by simpa using (prob_le_one (μ := P) (s := _)))
...
theorem exists_tendstoInProbability_of_prob_tendsto_zero {Ω : Type u_1} [MeasurableSpace Ω]
    ...
    ∃ (θ: ℕ → Ω → ℝ), TendstoInProbability θ P θ₀:= by
  unfold TendstoInProbability
  unfold TendstoInMeasure
  simp only
  -- pick a_n = 1/(n+1)
...
theorem exists_tendstoInProbability_of_prob_tendsto_zero'
  ...
  (h : ∀ (a : ENNReal), (0 < a) ∧ (a < ⊤) →
      ∃ (θ : ℕ → Ω → ℝ),
        Tendsto (fun i => P { ω | a ≤ edist (θ i ω) θ₀ }) atTop (𝓝 0)) :
  ∃ (θ : ℕ → Ω → ℝ), TendstoInProbability θ P θ₀ :=
by
  -- reduce to your original theorem by manufacturing the missing `a = ⊤` case
  refine
    exists_tendstoInProbability_of_prob_tendsto_zero
      (P := P) (θ₀ := θ₀) ?_
  intro a ha
  by_cases htop : a = ⊤
  · -- a = ⊤: the event is empty because `edist` on ℝ is never ⊤
    refine ⟨fun _ _ => θ₀, ?_⟩
    subst htop
    have hempty :
        ∀ (i : ℕ), {ω : Ω | (⊤ : ENNReal) ≤ edist ((fun _ _ => θ₀) i ω) θ₀} = (∅ : Set Ω) := by
      intro i
      ext ω
      -- `⊤ ≤ x` ↔ `x = ⊤`, and `edist_ne_top` kills it
      simp only [edist_self, nonpos_iff_eq_zero, ENNReal.top_ne_zero, Set.setOf_false,
        Set.mem_empty_iff_false]
    -- now it's just `Tendsto (fun _ => 0) ...`
    simp only [edist_self, nonpos_iff_eq_zero, ENNReal.top_ne_zero, Set.setOf_false, measure_empty,
      tendsto_const_nhds_iff]
  · -- a ≠ ⊤: now we can use h
    have ha_lt_top : a < ⊤ :=by
      exact Ne.lt_top' fun a_1 ↦ htop (id (Eq.symm a_1))
    exact h a ⟨ha, ha_lt_top⟩
